  ...
  return (
    <div className="min-h-screen">
      ...
      <main className="max-w-7xl mx-auto px-4 md:px-8 py-4">
        ...
        {activeTab === 'calendar' && (<div className="glass-card p-4 sm:p-6"><div className="flex items-center justify-between mb-4"><h2 className="text-2xl font-bold text-gray-800">{currentDate.toLocaleDateString('en-US', { month: 'long', year: 'numeric' })}</h2><div className="flex items-center gap-2"><button onClick={() => navigateMonth(-1)} className="p-2 hover:bg-white hover:bg-opacity-50 rounded-lg transition-colors"><ChevronLeft size={20} /></button><button onClick={() => navigateMonth(1)} className="p-2 hover:bg-white hover:bg-opacity-50 rounded-lg transition-colors"><ChevronRight size={20} /></button></div></div><div className="grid grid-cols-7 bg-white bg-opacity-30 rounded-t-lg">{['S', 'M', 'T', 'W', 'T', 'F', 'S'].map(day => <div key={day} className="p-2 text-center font-semibold text-gray-600 text-sm">{day}</div>)}</div><div className="grid grid-cols-7 grid-rows-6">{generateCalendarDays().map((dayObj, index) => { const dayEntries = getDayEntries(dayObj.date); const totalHours = getDayTotalHours(dayObj.date); return (<div key={index} className={`p-2 h-24 border-r border-b border-white border-opacity-20 ${!dayObj.isCurrentMonth ? 'bg-gray-50 bg-opacity-20' : ''} ${isToday(dayObj.date) ? 'bg-sky-100 bg-opacity-70' : ''}`}><div className={`text-sm font-semibold mb-1 ${!dayObj.isCurrentMonth ? 'text-gray-400' : isToday(dayObj.date) ? 'text-sky-600' : 'text-gray-700'}`}>{dayObj.date.getDate()}</div>{totalHours > 0 && (<div className="space-y-1"><div className="text-xs font-bold text-center text-white bg-sky-500 rounded-full py-0.5">{totalHours.toFixed(1)}h</div></div>)}</div>); })}</div></div>)}

        {/* --- MODALS with new glass style and text color fix --- */}
        {showAddSkill && (<div className="fixed inset-0 bg-black bg-opacity-60 backdrop-blur-sm flex items-center justify-center p-4 z-50"><div className="glass-card p-8 w-full max-w-md"><h3 className="text-2xl font-bold text-gray-800 mb-6">Create a New Skill</h3><div className="space-y-4">
          <div><label className="block text-sm font-semibold text-gray-700 mb-2">Skill Name</label><input type="text" value={newSkill.name} onChange={(e) => setNewSkill({...newSkill, name: e.target.value})} className="w-full p-3 bg-white bg-opacity-50 border border-white border-opacity-30 rounded-xl text-gray-900 focus:ring-2 focus:ring-sky-400" placeholder="e.g., Learn 3D Modeling"/></div>
          <div><label className="block text-sm font-semibold text-gray-700 mb-2">Category</label><input type="text" value={newSkill.category} onChange={(e) => setNewSkill({...newSkill, category: e.target.value})} className="w-full p-3 bg-white bg-opacity-50 border border-white border-opacity-30 rounded-xl text-gray-900 focus:ring-2 focus:ring-sky-400" placeholder="e.g., Creative"/></div>
          <div><label className="block text-sm font-semibold text-gray-700 mb-2">Daily Goal (hours)</label><input type="number" step="0.5" min="0.5" value={newSkill.targetHours} onChange={(e) => setNewSkill({...newSkill, targetHours: parseFloat(e.target.value)})} className="w-full p-3 bg-white bg-opacity-50 border border-white border-opacity-30 rounded-xl text-gray-900 focus:ring-2 focus:ring-sky-400"/></div>
          <div><label className="block text-sm font-semibold text-gray-700 mb-2">Pick a Color</label><div className="flex gap-2 flex-wrap">{colors.map(color => <button key={color} onClick={() => setNewSkill({...newSkill, color})} className={`w-8 h-8 rounded-full ${color} transition-transform duration-200 hover:scale-110 ${newSkill.color === color ? 'ring-4 ring-offset-2 ring-sky-500' : ''}`} />)}</div></div>
        </div><div className="flex gap-3 mt-8"><button onClick={() => setShowAddSkill(false)} className="flex-1 py-3 bg-white bg-opacity-80 rounded-xl font-semibold text-gray-700 hover:bg-opacity-100">Cancel</button><button onClick={addSkill} className="flex-1 py-3 bg-gradient-to-r from-sky-500 to-indigo-500 text-white rounded-xl font-semibold hover:shadow-lg">Add Skill</button></div></div></div>)}
        
        {showAddEntry && selectedSkill && (<div className="fixed inset-0 bg-black bg-opacity-60 backdrop-blur-sm flex items-center justify-center p-4 z-50"><div className="glass-card p-8 w-full max-w-md"><h3 className="text-2xl font-bold text-gray-800 mb-2">Log Your Progress</h3><p className="text-gray-600 mb-6">For: <span className="font-semibold">{selectedSkill.name}</span></p><div className="space-y-4">
